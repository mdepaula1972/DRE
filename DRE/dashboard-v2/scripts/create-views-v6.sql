-- SQL v6 - Força recriação completa das views
...
-- 2. Recriar contracts_expanded com AMBOS os tipos de empréstimo
...
field_contracts AS (
    SELECT 
        ...
        ((emp.loan_start_cycle || '-01')::date + (emp.loan_installments || ' months')::interval)::date as end_date,
        GREATEST(0, 
...
SELECT
    ...
    COALESCE((SELECT to_char(end_date, 'Mon/YY') FROM contracts_expanded WHERE remaining_installments > 0 ORDER BY end_date ASC LIMIT 1), '-') as proximo_encerrar,
...
SELECT 'Total contratos:' as info, COUNT(*) as value FROM contracts_expanded;
SELECT 'Contratos ativos:' as info, COUNT(*) as value FROM contracts_expanded WHERE remaining_installments > 0;
