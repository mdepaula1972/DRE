...
def sync_single_id(omie_id, empresa_nome, app_key_env, app_sec_env):
    ...
    if "faultstring" in r:
        print(f"Erro Omie: {r['faultstring']}")
        return

    # Usando data_entrada como sugerido para permitir sua correção manual
    ...
    print(f"Update Supabase: {resp.status_code}")
    print(f"Nova Data de Referência salva: {data_ref}")

# MarBrasil (pelo ID parece ser MarBrasil)
