-- SQL CORRIGIDO v4 - Inclui empréstimos de loan_amount (campos separados)
...
-- View: Contratos combinados (do JSON + campos separados)
CREATE VIEW contracts_expanded AS
-- Contratos do JSON loans_data
SELECT 
...
-- Contratos dos campos separados (loan_amount, loan_installments, loan_start_cycle)
SELECT 
    gen_random_uuid() as id,
    emp.id as employee_id,
    emp.corporate_name as employee_name,
    ...
    ((emp.loan_start_cycle)::text || '-01')::date as start_date,
    ...
    (((emp.loan_start_cycle)::text || '-01')::date + (emp.loan_installments::int || ' months')::interval)::date as end_date,
    GREATEST(0, 
        emp.loan_installments - 
        GREATEST(0, 
            (EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM ((emp.loan_start_cycle)::text || '-01')::date)) * 12 +
            (EXTRACT(MONTH FROM CURRENT_DATE) - EXTRACT(MONTH FROM ((emp.loan_start_cycle)::text || '-01')::date))
        )
    )::int as remaining_installments,
    LEAST(
        emp.loan_installments,
        GREATEST(0, 
            (EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM ((emp.loan_start_cycle)::text || '-01')::date)) * 12 +
            (EXTRACT(MONTH FROM CURRENT_DATE) - EXTRACT(MONTH FROM ((emp.loan_start_cycle)::text || '-01')::date)) + 1
        )
    )::int as installments_paid,
    'ATIVO' as status,
...
WHERE emp.loan_amount IS NOT NULL 
  ...
  AND (emp.loans_data IS NULL OR emp.loans_data = '[]' OR emp.loans_data = 'null');
