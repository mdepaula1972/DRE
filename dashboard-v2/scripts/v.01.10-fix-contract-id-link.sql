-- v.01.10 - Fix: Usar employee_id como contract_id para vincular parcelas
-- Problema: contracts_expanded gera UUIDs aleatórios, mas loan_payments usa employee_id

-- ========================================
-- CORREÇÃO: Recriar views com employee_id como contract_id
-- ========================================
...
-- Recriar contracts_expanded com employee_id como ID do contrato
CREATE OR REPLACE VIEW contracts_expanded AS
WITH json_contracts AS (
    SELECT 
        emp.id as id,  -- Usar employee_id como contract_id
...
contract_stats AS (
    ...
    LEFT JOIN loan_payments lp ON lp.contract_id = ac.id  -- Agora bate com employee_id
    GROUP BY ac.id
)
SELECT 
    ac.*,
...
-- Verificar se agora os contratos estão vinculados
SELECT 
    'Contratos com parcelas' as info,
