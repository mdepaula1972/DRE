-- SQL v5 - SIMPLIFICADO: Inclui ambos os tipos de empréstimo
...
-- View: Contratos do JSON (loans_data)
...
-- Contratos dos campos separados (quando não existe no JSON)
SELECT 
    gen_random_uuid() as id,
    emp.id as employee_id,
    emp.corporate_name as employee_name,
...
WHERE emp.loan_amount IS NOT NULL 
  ...
  AND (emp.loans_data IS NULL OR emp.loans_data = '[]' OR jsonb_array_length(emp.loans_data) = 0 OR emp.loans_data = 'null');
