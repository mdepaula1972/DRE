...
import { SlidersHorizontal, ArrowRight, RotateCcw, Target, Lightbulb, TrendingUp, TrendingDown } from 'lucide-react';
...
export function DreSimulator({ isOpen, params, onChange, onReset, originalResults, simulatedFcl }: DreSimulatorProps) {
  ...
  return (
    <div className="mb-8 p-5 bg-indigo-50/50 border border-indigo-100 rounded-2xl animate-in fade-in slide-in-from-top-4 duration-300">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2 text-indigo-800">
          <SlidersHorizontal size={20} />
          <h3 className="font-bold text-lg">Simulador de Cenários</h3>
        </div>
        <div className="flex gap-2">
          <button 
            onClick={() => setTargetMode(!targetMode)}
            className={`flex items-center gap-2 text-sm font-bold px-4 py-2 rounded-xl transition-colors shadow-sm ${
              targetMode ? 'bg-amber-500 text-white hover:bg-amber-600' : 'bg-white border border-indigo-200 text-indigo-600 hover:bg-indigo-50'
            }`}
          >
          ...
          <button 
            ...
            className="flex items-center gap-2 text-sm font-bold text-indigo-600 hover:text-indigo-800 bg-white px-4 py-2 rounded-xl border border-indigo-200 shadow-sm transition-colors"
          >
      ...
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 ${targetMode ? 'opacity-50 pointer-events-none' : ''}`}>
      ...
      <div className="mt-5 p-4 bg-white/80 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 border border-indigo-200">
        <div className="flex items-start gap-3">
          <div className="mt-1 text-indigo-500">
            <ArrowRight size={20} />
          </div>
          <div>
            <p className="text-sm text-indigo-900 font-semibold">Impacto Projetado no FCL Agregado</p>
            <p className="text-xs text-indigo-700/80">Esta simulação não altera os dados reais do CSV.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          <div className="text-right hidden sm:block">
            ...
            <p className="text-sm font-mono text-slate-700 line-through">
              {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(originalFcl)}
            </p>
          </div>
          <div className="text-right">
            <p className="text-xs font-bold text-indigo-600">Simulado</p>
            <p className={`text-lg font-black font-mono ${simulatedFcl >= originalFcl ? 'text-emerald-600' : 'text-rose-600'}`}>
