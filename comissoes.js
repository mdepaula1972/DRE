...
async function initOfflineMode(errorMsg) {
    ...
    try {
        ...
        // Guard: se a resposta for HTML (servidor retornou erro/index), abortar
        if (text.trim().startsWith('<!')) {
            throw new Error("Servidor local não encontrou o CSV. Rode o sistema via INICIAR_SISTEMA.bat.");
        }
        ...
        output.innerHTML = `❌ <strong>Erro Crítico:</strong> Não foi possível conectar à API nem carregar o CSV local.<br>
            <span class="text-warning">${err.message}</span><br><br>
            <strong>Solução:</strong> Inicie o sistema via <code>INICIAR_SISTEMA.bat</code> ou use a versão online com Supabase configurado.`;
